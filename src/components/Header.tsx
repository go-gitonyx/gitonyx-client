...
function MenuLabel({ label }: { label: string }) {
  return <div className="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-500">{label}</div>
}

function BranchMenu({ branch, branches, ahead, behind, loading, onCheckout }: { branch: string; branches: Branch[]; ahead: number; behind: number; loading: boolean; onCheckout: (branch: Branch) => void }) {
  const [open, setOpen] = useState(false)

  const locals = branches.filter((b) => !b.remote)
  const localNames = new Set(locals.map((b) => b.name))
  const remotes = branches.filter((b) => b.remote && !localNames.has(b.localName))
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {ahead > 0 && <span className="text-emerald-400 font-mono">↑{ahead}</span>}
        {behind > 0 && <span className="text-amber-400 font-mono">↓{behind}</span>}
          ...
          <div className="absolute left-0 top-full mt-2 z-20 w-72 max-h-80 overflow-y-auto rounded-xl bg-surface border border-line shadow-2xl py-1">
            ...
            {locals.map((b) => (
              ...
              >
                ...
                {b.current && <Icon icon="mdi:check" className="ml-auto text-primary shrink-0" />}
              </button>
            ))}
            ...
            {remotes.map((b) => (
              ...
              >
                <Icon icon="mdi:cloud-outline" className="text-accent shrink-0" />
                <span className="truncate">{b.name}</span>
              </button>
            ))}
          </div>
...
export function Header({ repoPath, branch, branches, ahead, behind, loading, authUser, onCheckout, onPull, onPush, onRefresh, onOpenAuth, onClose }: HeaderProps) {
  ...
  return (
    <header className="glass rounded-xl px-4 py-3 flex items-center gap-4">
      ...
      <div className="min-w-0">
        ...
        <div className="text-xs text-slate-500 font-mono truncate">{repoPath}</div>
      </div>

      <BranchMenu branch={branch} branches={branches} ahead={ahead} behind={behind} loading={loading} onCheckout={onCheckout} />

      <div className="ml-auto flex items-center gap-2">
        {loading && <Icon icon="mdi:loading" className="text-primary text-lg animate-spin" />}
        <ActionButton icon="mdi:arrow-down" label="Pull" onClick={onPull} disabled={loading} />
        <ActionButton icon="mdi:arrow-up" label="Push" onClick={onPush} disabled={loading} />
        <ActionButton icon="mdi:refresh" label="Refresh" onClick={onRefresh} disabled={loading} />
        ...
        <button
          ...
          className="size-9 rounded-lg glass flex items-center justify-center text-slate-400 hover:text-white hover:border-primary/40 transition-colors cursor-pointer"
        >
          <Icon icon={authUser ? 'mdi:account-check' : 'mdi:account-key-outline'} className={`text-lg ${authUser ? 'text-emerald-400' : ''}`} />
        </button>
      </div>
    </header>
  )
